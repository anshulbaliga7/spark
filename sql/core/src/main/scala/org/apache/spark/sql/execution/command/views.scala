...
object ViewHelper extends SQLConfHelper with Logging with CapturesConfig {
  ...
  def checkCyclicViewReference(
    ...
    // Detect cyclic references from subqueries nested at any depth in an expression tree.
    // The previous shallow match only caught a SubqueryExpression at the expression root and
    // missed subqueries inside larger expressions (e.g. a scalar subquery under an equality
    // predicate). Use foreach to walk every node in each expression tree. See SPARK-58894.
    plan.expressions.foreach(_.foreach {
      case s: SubqueryExpression => checkCyclicViewReference(s.plan, path, viewIdent)
      case _ => // Do nothing.
    })
  }
